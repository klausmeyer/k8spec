...
Node = Struct.new(:name) do
  def exists?
    $k8s.api('v1').resource('nodes').list.any? { |node| node.metadata[:name] == name}
  end
end

Namespace = Struct.new(:name) do
  def exists?
    $k8s.api('v1').resource('namespaces').list.any? { |namespace| namespace.metadata[:name] == name}
  end
end
...
Deployment = Struct.new(:namespace, :name) do
  ...
  def has_pods?(pods_spec = [])
    ...
    return false if pods.count { |p| p.dig(:status, :phase) == pods_spec.fetch(:state, 'Running') } < pods_spec.fetch(:count, 1)
    true
  end
end
...
RSpec::Matchers.define :have_a_deployment do |name|
  match do |namespace|
    deployment = Deployment.new(namespace.name, name)

    deployment.exists? && deployment.has_pods?(@pods_spec)
  end

  description do
    "have a deployment #{name} with pods: #{@pods_spec.inspect.gsub(/[{}:">]/, '')}"
  end

  chain :with_pods do |pods_spec|
    @pods_spec = pods_spec
  end
end

RSpec::Matchers.define :have_a_statefulset do |name|
  match do |namespace|
    sfs = Statefulset.new(namespace.name, name)

    sfs.exists? && sfs.has_pods?(@pods_spec)
  end

  description do
    "have a statefulset #{name} with pods: #{@pods_spec.inspect.gsub(/[{}:">]/, '')}"
  end
